<?php require_once __DIR__ . '/../../config.php'; ?>
<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>登録完了 - ラインストライク デッキビルダー</title>
    <!-- TODO: ファビコンを追加する -->
    <link rel="stylesheet" href="<?= BASE_URL ?>css/styles.css">
</head>
<body>
    <header>
        <a href="<?= BASE_URL ?>index.php" class="site-title">ラインストライク デッキビルダー</a>
        <h1 class="page-title">登録完了</h1>
        <nav class="header-nav">
            <a href="<?= BASE_URL ?>View/admin/admin_users.php" class="nav-button admin-btn">管理者ページへ</a>
            <a href="<?= BASE_URL ?>index.php" class="nav-button">トップページ</a>
            <a href="<?= BASE_URL ?>View/deck/construction.php" class="nav-button">新規デッキ登録</a>
            <a href="<?= BASE_URL ?>View/my_page/my_page.php" class="nav-button">マイデッキ保管庫</a>
        </nav>
    </header>

    <nav class="breadcrumb">
        <a href="<?= BASE_URL ?>index.php">トップページ</a>
        <span class="breadcrumb-separator">></span>
        <a href="register.php">ユーザー登録</a>
        <span class="breadcrumb-separator">></span>
        <span class="breadcrumb-current">登録完了</span>
    </nav>

    <div class="complete-container">
        <p class="complete-message">ユーザー登録が完了しました</p>
        <a href="<?= BASE_URL ?>index.php" class="complete-button">トップページへ</a>
    </div>

    <footer>
        <p class="footer-copyright">&copy; 2025 ラインストライク デッキビルダー</p>
    </footer>
</body>
</html>
